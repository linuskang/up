...
const STATUS_RANGE_COLORS = ["var(--chart-1)", "var(--chart-2)", "var(--chart-3)", "var(--chart-4)"] as const;
...
export default function AnalyticsPage() {
    ...
    const { statusData, statusChartConfig } = useMemo(() => {
        ...
        for (const range of ranges) {
            ...
            if (count > 0) {
                ...
                config[range] = { label: range, color: STATUS_RANGE_COLORS[colorIdx] };
                colorIdx++;
            }
        }
    ...
    const requestsThisWeek = useMemo(() => {
        ...
        return requestLogs.filter((log) => new Date(log.createdAt) >= startOfWeek).length;
    }, [requestLogs]);
    ...
    const endpointChartConfig = useMemo(() => {
        ...
        endpointData.forEach((item, i) => {
            config[item.endpoint] = { label: item.endpoint, color: `var(--chart-${(i % 5) + 1})` };
        });
        return config;
    }, [endpointData]);
        ...
        <main>
            <div className="flex min-h-svh flex-col gap-3 py-6">
                ...
                <div className="min-w-0">
                    <h1 className="truncate text-2xl font-bold">{project?.name} Analytics</h1>
                </div>

                <div className="flex flex-col gap-3">
                    <div className="grid grid-cols-3 gap-3">
                        <div className="rounded-lg bg-muted/40 p-3">
                            <p className="text-sm font-semibold text-muted-foreground">Weekly Requests</p>
                        ...
                        <div className="rounded-lg bg-muted/40 p-3">
                            ...
                            <p className="text-xl font-bold text-foreground">
                                {requestLogs.length > 0
                                    ? `${(
                                        (requestLogs.filter((log) => log.status >= 200 && log.status < 300).length /
                                            requestLogs.length) *
                                        100
                                    ).toFixed(2)}%`
                                    : "N/A"
                ...
                <Card className="ring-0">
                    ...
                    <CardContent>
                        <ChartContainer config={methodChartConfig} className="aspect-auto h-[250px] w-full">
                            <BarChart accessibilityLayer data={methodData} margin={{ left: -20 }}>
                                <CartesianGrid vertical={false} />
                ...
                <Card className="ring-0">
                    ...
                    <CardContent className="p-0">
                        <ChartContainer config={endpointChartConfig} className="mx-auto aspect-square max-h-[250px]">
                            <PieChart>
                                <ChartTooltip cursor={false} content={<ChartTooltipContent />} />
                                <Pie data={endpointData} dataKey="count" nameKey="endpoint" innerRadius={50} strokeWidth={2} labelLine={false}>
                ...
                <Card className="ring-0">
                    ...
                    <CardContent className="p-0">
                        <ChartContainer config={statusChartConfig} className="mx-auto aspect-square max-h-[250px]">
                            <PieChart>
                                <ChartTooltip cursor={false} content={<ChartTooltipContent />} />
                                <Pie data={statusData} dataKey="count" nameKey="status" innerRadius={50} strokeWidth={2} labelLine={false}>
